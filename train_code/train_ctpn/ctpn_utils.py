...
def resize(image, width=None, height=None, inter=cv2.INTER_AREA):
    """
    调整图像大小，保持宽高比。如果未指定宽度和高度，则返回原图像。
...
def gen_anchor(featuresize, scale):
    ...
    """
    heights = [11, 16, 23, 33, 48, 68, 97, 139, 198, 283]  # 锚点的高度列表
    widths = [16, 16, 16, 16, 16, 16, 16, 16, 16, 16]  # 锚点的宽度列表

    #切换形状变成变成列向量
    heights = np.array(heights).reshape(len(heights), 1)
    widths = np.array(widths).reshape(len(widths), 1)

    base_anchor = np.array([0, 0, 15, 15])  # 基础锚点 [x1, y1, x2, y2]，随便设定
    ...
    # 还原到特征图上的位置
    #在特征图上的锚点 需要在真实的图像上对齐
    #获取特征图上的长和宽
    h, w = featuresize
    #用np.arrange创建每个的索引 再乘scale，特征图上的点对应真实图像上
    shift_x = np.arange(0, w) * scale
    shift_y = np.arange(0, h) * scale

    # 将偏移（在原始图像上）应用到锚点上，生成所有锚点
    anchor = []
    # 特征图上的每个像素点
    for i in shift_y:
        for j in shift_x:
...
def cal_iou(box1, box1_area, boxes2, boxes2_area):
    ...
    - iou: 交并比（IoU）数组。
    """
    x1 = np.maximum(box1[0], boxes2[:, 0])  # 计算交集区域的左上角 x 坐标
    x2 = np.minimum(box1[2], boxes2[:, 2])  # 计算交集区域的右下角 x 坐标
    y1 = np.maximum(box1[1], boxes2[:, 1])  # 计算交集区域的左上角 y 坐标
    y2 = np.minimum(box1[3], boxes2[:, 3])  # 计算交集区域的右下角 y 坐标




    intersection = np.maximum(x2 - x1, 0) * np.maximum(y2 - y1, 0)  # 交集面积
    iou = intersection / (box1_area + boxes2_area[:] - intersection[:])  # 计算IoU
    return iou
...
def cal_overlaps(boxes1, boxes2):
    ...
    - overlaps: 交并比（IoU）矩阵。
    """
    area1 = (boxes1[:, 0] - boxes1[:, 2]) * (boxes1[:, 1] - boxes1[:, 3])  # 计算第一组边界框的面积
    area2 = (boxes2[:, 0] - boxes2[:, 2]) * (boxes2[:, 1] - boxes2[:, 3])  # 计算第二组边界框的面积

    overlaps = np.zeros((boxes1.shape[0], boxes2.shape[0]))  # 初始化交并比矩阵 行和列分别为BOX1和BOX2元素个数



    # 计算 boxes1（锚点）和 boxes2（GT box）的交并比，直接算出box1[i]和box2所有的交并比 再继续
...
def bbox_transfor_inv(anchor, regr):
    """
    将预测的垂直坐标变换还原为实际的边界框。
    bbox_transfor_inv 就是将差距再应用到锚框中 进行学习
...
def cal_rpn(imgsize, featuresize, scale, gtboxes):
    ...
    #base_anchor.shape 为 (N,4)N个框 每个框四个值（坐标）
    labels = np.empty(base_anchor.shape[0])
    labels.fill(-1)

    # 为每个 GT box 分配一个与其 IoU 最大的锚点，沿着行搜寻 一行为一个交并比
    ...
    # 负样本数量大于 RPN_TOTAL_NUM - 正样本数量 时 如果超了就设成-1（变成随机采样）
    if not OHEM:
...
def nms(dets, thresh):
    ...
    order = scores.argsort()[::-1]  # 根据得分从高到低排序，按照序号排序【在数组位置】
    ...
    while order.size > 0:
        ...
        inds = np.where(ovr <= thresh)[0]  # 保留IoU小于阈值的边界框，np.where返回的是元组，用[0]返回真正的数组
        order = order[inds + 1]
    return keep
...
class TextLineCfg:
    ...
    MAX_HORIZONTAL_GAP = 60  # 最大水平间隙
    TEXT_PROPOSALS_MIN_SCORE = 0.7  # 文本提案的最小得分
...
class TextProposalConnectorOriented:
    ...
    def get_text_lines(self, text_proposals, scores, im_size):
        ...
        for index, tp_indices in enumerate(tp_groups):
            ...
            text_lines[index, 5] = z1[0]  # 拟合直线的斜率k
            text_lines[index, 6] = z1[1]  # 拟合直线的截距b
